...
| `tools/watch_what_is_owed.py` | ONE WATCHER OVER WHAT A SESSION OWES — unpushed commits, an uncommitted tree, a changelog behind the shipped code, the four gates, and CI's colour ON THE HEAD rather than on the branch. Every check is a question put to git, the remote or a checker that already exists, never a search for a token; it re-derives everything each pass and EXITS when the answer changes, so the harness's own notification carries the finding. Hand-run it with `--once` from the repository before arming it. |
| `tools/watchdog.py` | Wraps a long command and tells SLOW from STUCK by CPU time rather than wall clock, sending SIGUSR1 first so a child that registered a faulthandler names the line it stopped on. Exit 125 for a stall, 124 for a timeout, both distinct from a test failure. |
...
| `.github/workflows/wintest.yml` | THE TARGETED WINDOWS RUN, a standing tool: installs QGIS as the `windows` job does and runs only the tests named at its foot — about five minutes against that job's ninety. Re-aim it by editing the run line, which is itself the trigger. Use it for a Windows-only red, and above all for a repair that cannot be judged here. |
| `.github/workflows/winprobe.yml` | THE WINDOWS PROBE WITHOUT QGIS, its sibling: a plain script under the runner's own python, answering in the time a runner takes to boot. For questions about the PLATFORM rather than the plugin — process and cpu accounting, path spellings, what a shell does. |
...
| **8-10** | `Topology`'s construction does its per-object work once: the match under a transform holds centroids and boxes (8), the symmetry filter compares against every unique at once (9), the two vertex scans ask one array distance each (10); identical structure, and a nine-strand whole-hole weave scaffold builds in 68 s where it never finished; retire all three with upstream's rewrite | `docs/process/study-upstream-topology-rewrite-2026-09-14.md` |
...
AND PATCHES 4-6 AND 8-10 ARE EXACT RATHER THAN MERELY FAST, so safe to
...
for 6, eleven topologies, a sabotage control and an oracle compiled from the anchor for 8-10 -- beside registered
