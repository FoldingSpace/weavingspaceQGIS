...
| `tools/doc_archive.py`, `docs/DOC-ARCHIVING.md` | The binding documents are split into a live half that carries the rules and an `-archived.md` half that carries the account behind each one, under ids the live half quotes. The tool checks that every pointer leads somewhere, that no account is stranded, and that each live document is inside its line budget; `--suggest` says what the next pass would look at. |
