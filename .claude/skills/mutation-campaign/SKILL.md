...
AND THE THIRD HALF, found the next day: the child needs the very
thing the module must not have. The macOS cask's interpreter carries
the paths of the machine it was built on and cannot start WITHOUT a
`PYTHONHOME`, so the guard added on 2026-08-19 -- probing `QGIS_PY`
and refusing when it cannot import qgis -- probed with the stripped
environment and refused every single time, reporting that QGIS_PY had
no QGIS when nobody had told it where its library lives. A guard that
can only refuse is the mirror of the instrument that could only
confirm. `macos_qgis_env.sh` exports the value twice now, the second
time as `QGIS_PYTHONHOME`, and `mutation_check.child_environment` puts
