...
def check_archived_documents():
  """Do the split binding documents still agree with their archives?

  Returns:
    None. Appends to the module-level ``problems``, like every other
    check here.

  CLAUDE.md, MAINTAINING.md, ROADMAP.md, docs/TESTING.md and
  docs/PUBLISHING.md keep their rules and quote an id for the account
  behind each one; the accounts live in `<NAME>-archived.md`. The
  reasoning and the procedure are in docs/DOC-ARCHIVING.md.

  IT IS RUN HERE BECAUSE THE SPLIT ROTS THE SAME WAY EVERY OTHER
  HAND-KEPT PAIRING IN THIS PROJECT ROTS. A pointer whose account was
  renumbered away sends a reader to nothing; an account nobody points
  at is a page that will be archived a second time by the next pass,
  because nothing records that it was already done. Neither shows up
  in a diff and both read as fine.

  AND THE BUDGET IS HERE FOR THE SAME REASON THE DERIVED DOCUMENTS ARE
  REGENERATED BY A GATE: the pass used to depend on somebody noticing
  that a document had got long, and the documents that most need it
  are exactly the ones a long session has no room left to read.
  """
  path = os.path.join(ROOT, "tools", "doc_archive.py")
  spec = _importlibutil.spec_from_file_location("_doc_archive", path)
  module = _importlibutil.module_from_spec(spec)
