...
def main():
  ...
  # The GUTTER is grey, not a rule around each cell. The maps are
  # drawn on white and several families leave white inside them --
  # the harbour, the space between icons -- so a cell needs something
  # to end against or a gap in the data reads as the space between two
  # designs. A drawn outline did that and added a sixteen-times
  # repeated frame the eye then has to ignore; colouring the space
  # between separates the cells with nothing added to them. Narrow,
  # because the grid should read as one object.
  # (User instruction, 2026-08-12: not a light grey line, the whole
  # space in between, and make it narrower.)
  # The outside margin is the SAME width as the gutters, so the frame
  # is even all the way round and the outer cells are not cropped
  # tighter than the inner ones. Without it the first and last columns
  # ran to the edge of the image while every other boundary had four
  # pixels of grey, which reads as a picture that has been trimmed.
  # (User instruction, 2026-08-12.)
  SIDE, GAP, GUTTER = 300, 4, "#bfbfbf"
  width = COLS * SIDE + (COLS + 1) * GAP
  height = ROWS * SIDE + (ROWS + 1) * GAP
  sheet = Image.new("RGB", (width, height), GUTTER)
  for index, png in enumerate(cells[:COLS * ROWS]):
    ...
    sheet.paste(tile, (GAP + col * (SIDE + GAP),
                       GAP + row * (SIDE + GAP)))
  sheet.save(OUT)
