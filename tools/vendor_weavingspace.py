...
def main():
  ...
  # PATCH 9: symmetries are de-duplicated against every unique at once, exactly.
  #
  # `Topology._remove_duplicate_symmetries` keeps a transform unless it is
  # `np.allclose` (atol and rtol 1e-4) to one already kept, and upstream asks
  # that with one call per candidate per unique. After patch 8 it was the
  # largest term left: MEASURED 2026-09-14 under QGIS 4.0.3, 106 of 239 cpu s
  # and 5.26 million calls on `plain weave abcdef|ghijkl` at 0.75 with whole
  # holes. `np.allclose(a, b)` is `all(abs(a - b) <= atol + rtol * abs(b))`, so
  # the same expression over the stacked uniques, row by row, is the same
  # verdict for every row -- the transforms are finite -- and the dictionary
  # keeps the same keys in the same order.
  # `test_a_symmetry_filter_compares_against_every_unique_at_once` holds it
  # to upstream's method, compiled from this anchor. Retire with upstream's
  # Topology rewrite, as patch 8.
  targeted(VENDOR_DIR / "topology.py", "9 symmetries de-duplicated at once",
  ...
  # PATCH 10: the two vertex scans ask one array distance, exactly.
  #
  # After patches 8 and 9, `Topology`'s construction still made 3.4 million
  # scalar `Point.distance` calls on `plain weave abcde|fghi` at 0.75 with
  # whole holes (MEASURED 2026-09-14 under QGIS 4.0.3): 15.7 cpu s placing
  # each patch corner onto a vertex already found (10a), and 9.6 s finding the
  # vertices incident on each tile's boundary (10b). `shapely.distance` over an
  # array is the GEOS call a Point's `.distance` makes; 10a keeps the vertices
  # in the order `self.points` grows, so the first hit is the vertex the loop's
  # `break` stopped at, and 10b keeps the dictionary's order and the
  # not-already-a-corner rule. Neither loop adds or moves a vertex between the
  # array's making and its reading except 10a's own additions, which it
  # appends. Held to the whole structure by
  # tools/probes/does_a_faster_match_build_the_same_topology.py and to
  # upstream's construction by `test_the_vertex_scans_ask_one_distance_each`.
  # Retire with upstream's Topology rewrite, as patches 8 and 9.
  targeted(VENDOR_DIR / "topology.py", "10a corners placed with one distance",
  ...
  targeted(VENDOR_DIR / "topology.py", "10a' the corner lookup's lists",
           '    self.tiles = []\n    self.points = {}\n    for (i, shape), label in zip(enumerate(shapes), labels, strict = True):\n',
           '    self.tiles = []\n    self.points = {}\n    import shapely\n    found_points, found_vertices = [], []\n    for (i, shape), label in zip(enumerate(shapes), labels, strict = True):\n')
  targeted(VENDOR_DIR / "topology.py", "10b incident vertices with one distance",
