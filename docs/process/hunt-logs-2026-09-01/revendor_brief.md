...
YOUR AREA: what the plugin reads from the vendored library now that it has moved bf1bbbf to 6190917

THE SHAPE YOU ARE HUNTING: unreachable.
A branch that cannot run because nothing upstream produces the state
it guards -- the condition is right and the precondition never
arrives. Distinguish carefully from a branch that is merely rare. The
evidence is in the callers, not in the branch.
