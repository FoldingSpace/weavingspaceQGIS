...
YOUR AREA: the topology tab's stores: the drawing, the dialog's own unit, the file's topology key, the change list and its validity marks

THE SHAPE YOU ARE HUNTING: two-stores.
One fact held in two places, where only one of them is updated or
persisted. The table and the layer; `_signature` and `_run_signature`;
the catalogue and the spin box. Ask which is the authority and what
happens when they disagree. Both defects found on 2026-08-13 by this
shape were cases where the disagreement did not stay cosmetic, because
the next Generate pushed the table's belief onto the map.
